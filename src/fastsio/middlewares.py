    ...
    async def __call__(
        ...
        **kwargs: Any
        ...
        try:
            ...
            return await self.handle_exception(e, event, sid, data, namespace, environ, auth, server, **kwargs)
    ...
    def __call__(
        ...
        **kwargs
        ...
        try:
            ...
            return self.handle_exception(e, event, sid, data, namespace, environ, auth, server, **kwargs)
        ...
        **kwargs
        ...
        for middleware in self.middlewares:
            ...
            if isinstance(middleware, SyncMiddleware):
                current_data = middleware.before_event(event, sid, current_data, namespace, environ, auth, server, **kwargs)
            else:
                current_data = await middleware.before_event(event, sid, current_data, namespace, environ, auth, server, **kwargs)
        ...
        for middleware in reversed(self.middlewares):
            ...
            if isinstance(middleware, SyncMiddleware):
                response = middleware.after_event(event, sid, response, namespace, environ, auth, server, **kwargs)
            else:
                response = await middleware.after_event(event, sid, response, namespace, environ, auth, server, **kwargs)
...
def auth_middleware(auth_checker: Callable[[str, Dict[str, Any]], bool]):
    ...
    class AuthMiddleware(BaseMiddleware):
        async def before_event(self, event: str, sid: str, data: Any, environ: Optional[Dict[str, Any]] = None, **kwargs):
