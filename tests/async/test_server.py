...
class TestAsyncServer:
    ...
    async def test_exception_handler(self, eio):
        ...
        async def handle_unicorn(
            sio: async_server.AsyncServer, sid: SocketID, exc: UnicornException
        ):
            assert sio is s
            return {"sid": sid, "name": exc.name}

        assert await s._trigger_event("boom", "/", "1") == {
            "sid": "1",
            "name": "spark",
        }

    async def test_router_exception_handler_only_applies_to_router(self, eio):
        class UnicornException(Exception):
            pass
        ...
        async def handled(sid):
            raise UnicornException()

        @unhandled_router.on("unhandled")
        async def unhandled(sid):
            raise UnicornException()

        @handled_router.exception_handler(UnicornException)
        async def handle_unicorn(exc: UnicornException):
            return "router"

        s.add_routers(handled_router, unhandled_router)

        assert await s._trigger_event("handled", "/", "1") == "router"
        with pytest.raises(UnicornException):
            await s._trigger_event("unhandled", "/", "1")

    async def test_router_exception_handlers_are_route_scoped(self, eio):
        class UnicornException(Exception):
            pass
        ...
        async def first_handler(exc: UnicornException):
            return "first"

        @second_router.exception_handler(UnicornException)
        async def second_handler(exc: UnicornException):
            return "second"

        s.add_routers(first_router, second_router)

        assert await s._trigger_event("boom", "/first", "1") == "first"
        assert await s._trigger_event("boom", "/second", "1") == "second"

    async def test_router_exception_handler_precedes_server_handler(self, eio):
        class UnicornException(Exception):
            pass
        ...
        @s.exception_handler(UnicornException)
        async def server_handler(exc: UnicornException):
            return "server"

        @router.exception_handler(Exception)
        async def router_handler(exc: Exception):
            return "router"

        s.add_router(router)

        assert await s._trigger_event("boom", "/", "1") == "router"

    async def test_exception_handler_uses_most_specific_exception_class(self, eio):
        ...
        async def app_handler(exc: AppException):
            return "app"

        @s.exception_handler(UnicornException)
        async def unicorn_handler(exc: UnicornException):
            return "unicorn"

        assert await s._trigger_event("boom", "/", "1") == "unicorn"

    async def test_unregistered_exception_is_reraised(self, eio):
        class UnicornException(Exception):
            pass
        ...
        with pytest.raises(UnicornException):
            await s._trigger_event("boom", "/", "1")

    async def test_server_exception_handler_catches_namespace_handler(self, eio):
        class UnicornException(Exception):
            pass
        ...
        assert await s._trigger_event("boom", "/chat", "1") == "namespace"

    async def test_exception_handler_does_not_catch_system_events(self, eio):
        class UnicornException(Exception):
            pass
        ...
        @s.exception_handler(UnicornException)
        async def handler(exc: UnicornException):
            return "handled"
        ...
        with pytest.raises(UnicornException):
            await s._trigger_event("connect", "/", "1")
        with pytest.raises(UnicornException):
            await s._trigger_event("disconnect", "/", "1")

    async def test_exception_handler_ack_response(self, eio):
        class UnicornException(Exception):
            pass
        ...
        s.eio.send.assert_awaited_once_with("123", '31000["handled"]')

    async def test_nested_router_exception_handlers_inherit_and_override(self, eio):
        class UnicornException(Exception):
            pass
        ...
        async def parent_handler(exc: UnicornException):
            return "parent"

        parent.add_router(child)
        s.add_router(parent)

        assert await s._trigger_event("boom", "/api/chat", "1") == "parent"
        ...
        @child.on("boom")
        async def boom_override(sid):
            raise UnicornException()

        @parent.exception_handler(UnicornException)
        async def parent_handler_override(exc: UnicornException):
            return "parent"

        @child.exception_handler(UnicornException)
        async def child_handler(exc: UnicornException):
            return "child"

        parent.add_router(child)
        s.add_router(parent)

        assert await s._trigger_event("boom", "/api/chat", "1") == "child"
